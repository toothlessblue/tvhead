...
static int getDisplay(EGLDisplay *display) {
    ...
    gbmSurface = gbm_surface_create(gbmDevice, mode.hdisplay, mode.vdisplay,
                                    GBM_FORMAT_XRGB8888,
...
static void gbmSwapBuffers(EGLDisplay *display, EGLSurface *surface) {
    ...
    uint32_t fb;
    drmModeAddFB(device, mode.hdisplay, mode.vdisplay, 24, 32, pitch, handle,
                 &fb);
...
static const GLfloat vertices[] = {-1.0f, -1.0f, 0.0f, 0.0f, 0.0f,
...
int initOpenGL() {
    ...
    // We will use the screen resolution as the desired width and height for the
    // viewport.
    int desiredWidth = 64;
    ...
    EGLConfig *configs =
        reinterpret_cast<EGLConfig *>(malloc(count * sizeof(configs)));
    ...
    if (viewport[2] != desiredWidth || viewport[3] != desiredHeight) {
        fprintf(stderr, "Error! The glViewport returned incorrect values! "
                        "Something is wrong!\n");
        eglDestroyContext(display, context);
